...
class ParserFindContentManagerSpec (TestCase):

    file_content: str
    error_manager: ErrorManager

    def __init__(self: ParserFindContentManagerSpec, methodName: str = "runTest") -> None:
        super().__init__(methodName)

        self.error_manager = ErrorManager(".", 0)
        self.file_content = read_safe(Path("../test/testdata/DoAction1.as"), self.error_manager)

    def test_resolve_instance_no_success(self: ParserFindContentManagerSpec) -> None:
        context = get_replace_patch_context(Path("../test/testdata/Patch3.patch"), 2)

        data, location = ParserFindContentManager(context, "test") \
    ...
    def test_resolve_function_success(self: ParserFindContentManagerSpec) -> None:
    ...
    def test_resolve_function_offset_success(self: ParserFindContentManagerSpec) -> None:
    ...
    def test_resolve_end_success(self: ParserFindContentManagerSpec) -> None:
    ...
    def test_resolve_nth_failure_too_large(self: ParserFindContentManagerSpec) -> None:
        context = get_add_patch_context(Path("../test/testdata/Patch1.patch"), 0)

        with raises(InjectionError):
            ParserFindContentManager(context, "test") \
                .resolve(self.file_content, self.error_manager)

    def test_resolve_function_failure_too_large(self: ParserFindContentManagerSpec) -> None:
    ...
    def test_resolve_function_failure_no_function(self: ParserFindContentManagerSpec) -> None:
    ...
    def test_resolve_end_failure_no_instance(self: ParserFindContentManagerSpec) -> None:
        context = get_add_patch_context(Path("../test/testdata/Patch1.patch"), 2)

        with raises(InjectionError):
            ParserFindContentManager(context, "derppotato1") \
                .resolve(self.file_content, self.error_manager)
